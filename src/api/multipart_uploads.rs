use crate::utils::{CHUNK_MAX_SIZE, CHUNK_MIN_SIZE, DATA_CACHES, DEFAULT_CHUNK_SIZE, FAST_FINALITY_INDEXES, RECEIPT_HEIGHT_DEADLINE, RECEIPT_VERSION};
use crate::s3::{create_s3_multipart, finalize_multipart_upload, upload_part_s3};
use crate::db::{get_chunks, get_completed_upload, get_upload, save_chunk, update_chunk_size, create_upload_record};
use crate::arbundles::{UnsignedReceipt, SignedReceipt, sign_receipt};
use axum::{
    extract::{Path, State},
...
pub async fn create_multipart_upload_handler(
    ...
    let s3_upload_id =  create_s3_multipart(&upload_key).await.map_err(|_e| StatusCode::INTERNAL_SERVER_ERROR)?;
    
    // store in db
    let _ = create_upload_record(&pool, &upload_id, &upload_key, &s3_upload_id).await.map_err(|_e| StatusCode::INTERNAL_SERVER_ERROR);
...
pub async fn get_multipart_upload_handler(
    ...
    let chunks = get_chunks(&pool, &upload_id).await
...
pub async fn post_chunk_handler(
    ...
    let content_length = headers
        ...
        .and_then(|v| v.parse::<usize>().ok()).ok_or(StatusCode::BAD_REQUEST)?;
    ...
    let chunk_size = if upload.chunk_size.is_none() || content_length as i64 > upload.chunk_size.unwrap_or(0) {
    ...
    let etag = upload_part_s3(
        &upload.upload_key,
        &upload.s3_upload_id,
        part_number as i32,
        body.to_vec()
    ).await.map_err(|_e| StatusCode::INTERNAL_SERVER_ERROR)?;
    
    let _ = save_chunk(&pool, &upload_id, part_number as i64, &etag, content_length as i64).await.map_err(|_e| StatusCode::INTERNAL_SERVER_ERROR)?;
        ...
        Err(_) => {
            ...
            match get_completed_upload(&pool, &upload_id).await {
                Ok((dataitem_id, owner_address)) => {
                    ...
                    let unsigned_receipt : UnsignedReceipt = UnsignedReceipt { id: dataitem_id, deadline_height: RECEIPT_HEIGHT_DEADLINE, timestamp: chrono::Utc::now().timestamp_millis() as u64, version: RECEIPT_VERSION.to_string(), owner: owner, data_caches: vec![DATA_CACHES.to_string()], fast_finality_indexes: vec![FAST_FINALITY_INDEXES.to_string()], winc: "0".to_string() };
                    let signed_receipt = sign_receipt(unsigned_receipt).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
                    let res = MultipartUploadStatus {status: "FINALIZED".to_string(), receipt: signed_receipt };
